# AGENTS.md — kobai-walk

勾配降下の地形歩き。損失地形の上を GD/Momentum/AdaGrad が降りるレースを描く教材アプリ。
正規方程式・臨界学習率 2/λmax の解析オラクル(G-02〜G-04)で検証する。仕様は SPEC.md、テストは TEST_SPEC.md。
...
- クライアント専用 API(document / window / canvas 生成)は **useEffect 内でのみ**触る。
  useMemo・レンダ本体で呼ぶと SSR プリレンダで落ちる(HC-002)。
- 可視化系の純関数は**非有限値(NaN / ±Inf)入力の挙動を仕様として定義**し core でテストする。
  発散を見せる UI では非有限値は異常系でなく正常系である(HC-002)。
